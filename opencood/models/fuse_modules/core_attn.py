...
from opencood.models.comm_modules.core import Communication
...
class Core(nn.Module):
    ...
    def forward(self, ego_features, sparse_features, sparse_indices, original_dims, out_channel, net_decompress, rm, record_len, pairwise_t_matrix, backbone=None):
        """
        Fusion forwarding.
        ...
        """
        x = sparse_features
        N, C, H, W = original_dims
        C_app = out_channel
        sp_voxel = torch.zeros((N, 1, H, W), device=x.device)
        for i in range(N):
            feature_temp = x[i]
            index_temp = sparse_indices[i]
            for j in range(len(feature_temp)):
                sp_voxel[i, 0, index_temp[j,0], index_temp[j,1]] = feature_temp[j]
        
        sp_voxel = net_decompress(sp_voxel)
        x = sp_voxel
