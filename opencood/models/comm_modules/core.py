...
class Communication(nn.Module):
    def __init__(self, args):
        ...
        self.compressor = nn.Sequential(
                            nn.Conv2d(args['anchor_num'], 1, kernel_size=1),
                            nn.Softmax(dim=1)
                )
    ...
    def forward(self, batch_confidence_maps, record_len, pairwise_t_matrix):
        ...
        for b in range(B):
            ...
            ori_communication_maps = batch_confidenc_maps[b]
            ones_mask = torch.ones_like(ori_communication_maps).to(ori_communication_maps.device)
            confidence_maps = ones_mask - ori_communication_maps
