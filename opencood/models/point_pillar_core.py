...
class pointpillarcore(nn.Module):
    def __init__(self, args):
        ...
        grid_size = (np.array(self.cav_lidar_range[3:6]) -
                     np.array(self.cav_lidar_range[0:3])) / np.array(self.voxel_size)
        self.grid_size = np.round(grid_size).astype(np.int64)
        norm_fn = partial(nn.BatchNorm2d, eps=1e-3, momentum=0.01)
        self.conv = nn.Sequential(
            nn.Conv2d(args['compress_core']['in_channels'], args['compress_core']['out_channels'], kernel_size=1, stride=1, padding=0, bias=False),
